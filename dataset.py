...
def label_smoothing(labels, epsilon=0.1):
    # labels: one-hot encoded labels with shape (batch_size, grid_size, grid_size, nb_anchors, nb_classes)
...
class SequenceData(Sequence):
    ...
    def read(self, dataset):
                        ...
                        for i in range(-1,2):
                            for j in range(-1,2):
                                if (0 < loc_i + i < X_div) and (0 < loc_k + j < Z_div):
                                    ...
                                    anchors = [[((loc_i+i) * x_step*cfg.factor)+self.x_min, a[3], ((loc_k+j) * z_step*cfg.factor)+self.z_min, a[0], a[1], a[2], a[4]] for a in self.anchor]
        ...
        # smoothed_conf_matrix = label_smoothing(conf_matrix, epsilon=0.1)
        # conf1 = np.dstack((conf_matrix[:,:,:2,0],np.zeros((X_div, Z_div, 1))))
        # conf2 = np.dstack((smoothed_conf_matrix[:,:,:2,0],np.zeros((X_div, Z_div, 1))))
