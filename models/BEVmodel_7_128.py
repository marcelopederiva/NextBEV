...
def depthwise_sep_block(entrylayer, channels, Ksize, stride, pad='same', group=1):
    # Depthwise Convolution
    c = DepthwiseConv2D(kernel_size=Ksize, strides=stride, padding=pad, use_bias=False)(entrylayer)
...
def BottleNeckBlock(entry_layer, in_features, out_features, exp, stride=1):
    ...
    conv_seq = depthwise_sep_block(entry_layer, in_features,
...
def Head(entry_layer):
    ...
    # last_class = Conv2D(256, (3,3), padding = 'same', name="class1", activation="relu")(entry_layer)
    ...
    # last_conf = Conv2D(256, (3,3), padding = 'same', name="occ1", activation="relu")(entry_layer)
    last_conf = depthwise_sep_block(entry_layer, 128, Ksize=(2,2), stride=(1,1), pad='same', group=1)
    ...
    # last_pos = Conv2D(256, (3,3), padding = 'same', name="loc1", activation="relu")(entry_layer)
    last_pos = depthwise_sep_block(entry_layer, 64, Ksize=(2,2), stride=(1,1), pad='same', group=1)
    ...
    # last_dim = Conv2D(256, (3,3), padding = 'same', name="size1", activation="relu")(entry_layer)
    last_dim = depthwise_sep_block(entry_layer, 64, Ksize=(2,2), stride=(1,1), pad='same', group=1)
    ...
    # last_rot = Conv2D(256, (3,3), padding = 'same', name="rot1", activation="relu")(entry_layer)
    last_rot = depthwise_sep_block(entry_layer, 32, Ksize=(2,2), stride=(1,1), pad='same', group=1)
...
def Img2BEV(img):
    ...
    # print(c5)
    # exit()
    c2 = conv_block(c2, 128, (3,3), (2,2), pad = 'same') # 128.128.256
    conc1 = Concatenate()([c2,c3])
    # conc2 = Concatenate()([c4,c5])
    # conc = Concatenate()([conc1,conc2])
    c_5_4 = Concatenate()([c4,c5]) # 64.64.512c
    c_54 = conv_block_up(c_5_4, 128, (3,3), (2,2)) # 128.128.256
    conc = Concatenate()([conc1,c_54])# 128.128.384c
    # c_2 = conv_block(c2, 128, (3,3), (2,2)) # 256.256.256
    # c2_543 = Concatenate()([c_2,c3_54])# 256.256.384c
    # print(conc.shape)
    # exit()
    out = LayerNormalization()(conc)
