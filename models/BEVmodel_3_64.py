...
def Img2BEV(img):
    ...
    C1_DW = depthwise_sep_block(C1, C1.shape[-1], (1,1), (1,1), pad='valid')
    C1_DW = MaxPool2D((2,2))(C1_DW)

    C2 = conv_block(C1, nb_channels, (3,3), (2,2), pad='same')
    C2_out = Concatenate(axis=-1)([C1_DW,C2]) # 256
    C2_DW = depthwise_sep_block(C2_out, C2_out.shape[-1], (1,1), (1,1), pad='valid')
    C2_DW = MaxPool2D((2,2))(C2_DW)

    C3 = conv_block(C2, nb_channels*2, (3,3), (2,2), pad='same')
    C3_out = Concatenate(axis=-1)([C2_DW,C3])
    C3_DW = depthwise_sep_block(C3_out, nb_channels, (1,1), (1,1), pad='valid')
    # C3_DW = MaxPool2D((2,2))(C3_DW)

    C4 = conv_block(C3, nb_channels, (3,3), (1,1), pad='same')
    C4_out = Concatenate(axis=-1)([C3_DW,C4])
    C4_out = depthwise_sep_block(C4_out, C4_out.shape[-1], (1,1), (1,1), pad='valid')


    # down C1
    C1_out = conv_block(C1, nb_channels*2, (3,3), (2,2), pad='same')
    C1_out = conv_block(C1_out, nb_channels*2, (3,3), (2,2), pad='same')

    # down C2
    C2_out = conv_block(C2_out, nb_channels*2, (3,3), (2,2), pad='same')

    # down C3
    C3_out = conv_block(C3_out, nb_channels*2, (3,3), (1,1), pad='same')
    ...
    # Mix everything
    out = Concatenate()([C1_out,C2_out,C3_out,C4_out])

    out = depthwise_sep_block(out, 64, (1,1), (1,1), pad='valid')
    return out
