...
def SimpleConv2D(entry_layer, out_features, Ksize, stride=1, group=1, pad='same'):
    ...
    return c
def depthwise_sep_block(entrylayer, channels, Ksize, stride, pad='same'):
...
def BottleNeckBlock(entry_layer, in_features, out_features, exp, stride=1):
    ...
    conv_seq = DepthwiseConv2D(kernel_size=5,strides = stride, use_bias = False, padding = 'same')(entry_layer)
    ...
    conv_seq = depthwise_sep_block(conv_seq, expanded_features, Ksize=1,stride=1)
    # conv_seq = SimpleConv2D(conv_seq, expanded_features, Ksize=1, stride=1)
    conv_seq = Activation(tf.nn.relu6)(conv_seq)

    # conv_seq = depthwise_sep_block(conv_seq, out_features, Ksize=1,stride=1)
...
def Head(entry_layer):
    ...
    # last_class = Conv2D(8, (1, 1), padding = 'valid', name="class1", activation="relu")(entry_layer)
...
def Img2BEV(img):
    c1 = depthwise_sep_block(img, channels=32,
                     Ksize=3, stride=1, pad='same')
    ...
    c2 = depthwise_sep_block(c2, channels=64,
                     Ksize=3, stride=2, pad='same')
    ...
    c3 = depthwise_sep_block(c3, channels=64,
                     Ksize=3, stride=2, pad='same')
    ...
    c4 = depthwise_sep_block(c4, channels=128,
                     Ksize=3, stride=2, pad='same')
    ...
    c5 = depthwise_sep_block(c5, channels=128,
                     Ksize=3, stride=1, pad='same')
